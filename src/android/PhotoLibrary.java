          ...
          public void run() {
            try {
              ...
              PhotoLibraryGetLibraryOptions getLibraryOptions = new PhotoLibraryGetLibraryOptions(itemsInChunk,
                  maxItems, chunkTimeSec, includeAlbumData, includeVideos);
        ...
        cordova.getThreadPool().execute(new Runnable() {
          public void run() {
            try {
              ...
              String thumbnailURL = service.getThumbnailURL(getContext(), mediaId, mimeType, fileName, thumbnailWidth,
                  thumbnailHeight, quality);

              callbackContext.sendPluginResult(new PluginResult(PluginResult.Status.OK, thumbnailURL));
